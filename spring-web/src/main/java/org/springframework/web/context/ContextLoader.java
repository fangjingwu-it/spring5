...
public class ContextLoader {
	 ...
	 * 实际的ApplicationContextInitializer实例应用于上下文：在应用上下文refresh之前调用该类型的回调
	 */
	 ...
	 * @param servletContext current servlet context 当前的web容器上下文
	...
	protected WebApplicationContext createWebApplicationContext(ServletContext sc) {
		...
		// 判定一个当前Class对象表示的类或者接口 与 传入的Class参数相比，是否相同，或者是否是它的一个超类或者超接口
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac, ServletContext sc) {
		...
		// 获取web.xml中配置的contextConfigLocation参数对应的值（值对应的是Spring应用上下文的配置文件）
		...
		/*
		 * Spring应用上下文被刷新之前，所有使用的环境(environment) 的initPropertySources方法将会被调用，
		 * 以确保后续的refresh()方法执行时刷新所使用的环境是正确的
		 *
		 * 初始化Spring应用的Environment环境---最先被执行
		 */
		...
		// 检查有没有自定义的上下文，即有没有需要对应用上下文做初始化时的个性化设置。
	 ...
	 *
	 * customizeContext是定制化conext的方法，如果没有在web.xml中配置contextInitializerClasses初始化参数则此方法不做任何工作
	 *
	 */
	protected void customizeContext(ServletContext sc, ConfigurableWebApplicationContext wac) {
		...
		// 对所有ApplicationContextInitializer的类型进行初始化
		for (ApplicationContextInitializer<ConfigurableApplicationContext> initializer : this.contextInitializers) {

			// 即会执行ApplicationContextInitialize#initialize()方法中的操作，先refresh Spring上下文进行操作
	...
	protected List<Class<ApplicationContextInitializer<ConfigurableApplicationContext>>>
		...
		// 到web.xml中中获取配置的初始化参数globalInitializerClasses对应的ApplicationContextInitializer类型的类
		...
		// 到web.xml中中获取配置的初始化参数contextInitializerClasses对应的ApplicationContextInitializer类型的类
